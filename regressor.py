...
horsepower_model.summary()#Model Özeti gösterir
...
x = tf.linspace(0.0, 250, 251)#BElirlenen aralıkta eşit sayıda sayı döner
...
print("************************************","COKLU GIRISLI DOGRUSAL REGRESYON")
...
linear_model.compile(
    optimizer=tf.optimizers.Adam(learning_rate=0.1),
    loss='mean_absolute_error') #Modeli compile eder büyük verirsek hızlı öğrenir ama overfitting olabilir küçük verirsek yavaş ve iyi öğrenir.
...
history = linear_model.fit(
    ...
    validation_split = 0.2)
plot_loss(history)
#Sonuçları hafızada tutar
test_results['linear_model'] = linear_model.evaluate(
    test_features, test_labels, verbose=0)

print("-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-DNN ile Regresyon")
...
print("-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-DNN ve Çoklu giriş ile Regresyon")
# Çoklu veri girişi ile regresyon yapılır.
